...
function updateUIFromCrop() {
  if (!selectionActive || cropW <= 0 || cropH <= 0) {
    inputX.disabled = true; inputY.disabled = true; inputW.disabled = true; inputH.disabled = true;
    sizeInfo.innerHTML = `⚡ select or start cropping`;
    if (cropW <= 0) selBox.style.display = 'none';
    updateLivePreview();
    return;
  }
  inputX.disabled = false; inputY.disabled = false; inputW.disabled = false; inputH.disabled = false;
  inputX.value = cropX;
  inputY.value = cropY;
  inputW.value = cropW;
  inputH.value = cropH;
...
function deactivateCrop() {
  ...
  selBox.style.display = 'none';
  inputX.disabled = true; inputY.disabled = true; inputW.disabled = true; inputH.disabled = true;
  inputX.value = 0; inputY.value = 0; inputW.value = 0; inputH.value = 0;
...
function getResizeHandleType(mouseCanvasX, mouseCanvasY) {
  ...
  if (mouseCanvasX > leftEdge && mouseCanvasX < rightEdge && mouseCanvasY > topEdge && mouseCanvasY < bottomEdge) return 'move';
  return null;
}

function setCursorFromHandle(handle) {
  if (!handle) { selBox.style.cursor = 'grab'; return; }
  const cursors = {
    move: 'grab', top: 'ns-resize', bottom: 'ns-resize', left: 'ew-resize', right: 'ew-resize',
...
function onGlobalMouseMove(e) {
  ...
  let newX = dragStartCrop.x, newY = dragStartCrop.y, newW = dragStartCrop.w, newH = dragStartCrop.h;
  
  switch(dragType) {
    case 'move': newX = dragStartCrop.x + deltaX; newY = dragStartCrop.y + deltaY; break;
    case 'right': newW = dragStartCrop.w + deltaX; break;
    case 'left': newX = dragStartCrop.x + deltaX; newW = dragStartCrop.w - deltaX; break;
    case 'bottom': newH = dragStartCrop.h + deltaY; break;
    case 'top': newY = dragStartCrop.y + deltaY; newH = dragStartCrop.h - deltaY; break;
    case 'topRight': newY = dragStartCrop.y + deltaY; newH = dragStartCrop.h - deltaY; newW = dragStartCrop.w + deltaX; break;
    case 'topLeft': newX = dragStartCrop.x + deltaX; newW = dragStartCrop.w - deltaX; newY = dragStartCrop.y + deltaY; newH = dragStartCrop.h - deltaY; break;
    case 'bottomRight': newW = dragStartCrop.w + deltaX; newH = dragStartCrop.h + deltaY; break;
    case 'bottomLeft': newX = dragStartCrop.x + deltaX; newW = dragStartCrop.w - deltaX; newH = dragStartCrop.h + deltaY; break;
  }
  if (newW < 2) newW = 2;
  if (newH < 2) newH = 2;
  setCrop(newX, newY, newW, newH, true);
...
function updateHeaderCoords(e) {
  ...
  if (canvasX >= 0 && canvasY >= 0 && canvasX <= mainCanvas.width && canvasY <= mainCanvas.height) {
    const orig = canvasToOriginalCoord(canvasX, canvasY);
    coordsEl.textContent = `x: ${orig.x}  y: ${orig.y}  |  w: —  h: —`;
  } else {
    if (selectionActive && cropW>0) coordsEl.textContent = `crop: ${cropW}×${cropH}`;
    else coordsEl.textContent = `x: —  y: —  |  w: —  h: —`;
  }
}

function onCanvasMouseLeave() {
  if (selectionActive && cropW>0) coordsEl.textContent = `crop active: ${cropW}×${cropH}`;
  else coordsEl.textContent = `x: —  y: —  |  w: —  h: —`;
}
...
cropBtn.addEventListener('click', () => { if (selectionActive && cropW>0) updateLivePreview(); });
...
newCropBtn.addEventListener('click', () => { if (img) startCropping(); });
